...
Confabula produce dei file HTML eseguibili su browser Web. All'avvio all'utente si presenta una descrizione principalmente testuale (Scena) con una casella di testo sottostante (Input) dove l'utente (Giocatore) può scrivere in prima persona o con l'imperativo le sue azioni. In base a ciò che l'utente scrive possono comparire risposte e può aggiornarsi la Scena.
...
Confabula è programmato in JavaScript 5 (più l'uso delle arrow functions di JavaScript 6), i contenuti e la grafica sono realizzati con HTML5 e CSS3.


## Albero delle cartelle e dei file
<!-- Capitolo 1 -->

Confabula ha una struttura gerarchica molto semplice. I file essenziali sono tutti presenti in un'unica cartella. Eventuali sotto cartelle sono utili per raccogliere immagini e/o suoni se ci sono. Di seguito l'elenco dei file essenziali.

### INIZIA.html
'INIZIA.html' è un file statico e non verrà mai modificato dallo Scrittore, solo eventuali cambi strutturali profondi (quindi rari) possono portare lo Sviluppatore a rivedere alcuni punti.
...
### scene.js
'scene.js' è il file su cui maggiormente lavorerà lo Scrittore, deve rimanere più semplice ed intuitivo possibile. Inizialmente ci sono degli array utili per definire il vocabolario (facoltativo) e le espressioni equivalenti (ovvero i sinonimi, ma non solo).

Segue una funzione costituita da una serie di istruzioni che avranno valore in tutte le scene del gioco. Le istruzioni sono definite tramite una funzione, ma ciascuna funzione serve solo a preparare le condizioni che ad ogni azione del giocatore saranno verificate per decidere se eseguire l'istruzione oppure no.
